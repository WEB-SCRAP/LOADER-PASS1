...
int check_H_ESTAB_Error(struct H_ESTAB Hrecord, struct H_ESTAB Hrecords[],
                        struct D_ESTAB Drecords[], int Hcount, int Dcount) {
  // rewind(fp2);
...
int check_D_ESTAB_Error(struct D_ESTAB Drecord, struct H_ESTAB Hrecords[],
                        struct D_ESTAB Drecords[], int Hcount, int Dcount) {
  // rewind(fp2);
...
// Process input file
int Process(FILE *fp1, FILE *fp2, struct H_ESTAB Hrecords[],
            struct D_ESTAB Drecords[], int PROGADDR) {
  int line_count = 0;
  int Hcount = 0;
  int Dcount = 0;
    ...
    if (line[0] == 'H' && line[1] == '^') {
      ...
      if (flag == 0) {
        ...
        // Next records continue after previous ones
          ...
          Hrecords[Hcount].address +=
              Hrecords[Hcount - 1].address + Hrecords[Hcount - 1].length;
        }

        // Writing in ESTAB file
        printf("%s\t%s\t%04X\t%04X\n", Hrecords[Hcount].csname,
               Hrecords[Hcount].extsym, Hrecords[Hcount].address,
               Hrecords[Hcount].length);
      ...
      for (int i = 2; i < strlen(line) + 1; i++) {
        ...
        if (line[i] == '^' || line[i] == '\0') {
          ...
          if (toggle == 0) {
            ...
            // Check for repeated name
            int flag =
                check_D_ESTAB_Error(Dtemp, Hrecords, Drecords, Hcount, Dcount);
            ...
            if (flag == 0) {
              ...
              printf("\t%s\t%04X\n", Drecords[Dcount].varname,
                     Drecords[Dcount].varaddress);
              fprintf(fp2, "\t%s\t%04X\n", Drecords[Dcount].varname,
                      Drecords[Dcount].varaddress);
              Dcount++;
            } else if (flag == 1) {
...
int main() {
  FILE *fp1 = fopen("input.txt", "r"); // Open input file in read mode
